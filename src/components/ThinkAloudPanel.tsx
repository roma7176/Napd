...
export function ThinkAloudPanel({ caseId }: { caseId: string }) {
  ...
  async function handleSubmit() {
    ...
    setError(false);
    try {
      const result = await submitThinkAloud(caseId, text);
        ...
        {entries.map((entry) => (
          <div key={entry.id} className="space-y-1.5">
            ...
            <div className="rounded-md border border-line bg-surface px-3 py-2">
              ...
              <p className="mt-1 text-sm leading-relaxed text-ink-soft">{entry.acknowledgement}</p>
            </div>
          </div>
        ))}
        {error && (
